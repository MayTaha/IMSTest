...
{
    public abstract class IMSWebTestBase : AbpAspNetCoreIntegratedTestBase<Startup>
    {
        protected static readonly Lazy<string> ContentRootFolder;

        static IMSWebTestBase()
        {
        ...
        protected override IWebHostBuilder CreateWebHostBuilder()
        {
            return base
                ...
                .UseSetting(WebHostDefaults.ApplicationKey, typeof(IMSWebMvcModule).Assembly.FullName);
        }
        ...
        protected void UsingDbContext(Action<IMSDbContext> action)
        {
            using (var context = IocManager.Resolve<IMSDbContext>())
            {
                action(context);
                context.SaveChanges();
            }
        ...
        protected T UsingDbContext<T>(Func<IMSDbContext, T> func)
        {
            T result;
        ...
        protected async Task UsingDbContextAsync(Func<IMSDbContext, Task> action)
        {
            using (var context = IocManager.Resolve<IMSDbContext>())
            {
        ...
        protected async Task<T> UsingDbContextAsync<T>(Func<IMSDbContext, Task<T>> func)
        {
            T result;
